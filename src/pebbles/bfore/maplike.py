...
class MapLike(object) :
    ...
    def get_amplitude_mean(self, spec_params, inst_params=None,
        ...
        """
        # Again, we're allowing F and N_T to be passed to avoid extra
        # operations. Should we be passing choleskys here?
        if f_matrix is None:
            f_matrix = self.f_matrix(spec_params, inst_params)
        # f_matrix -> (N_comp,N_freq)
        if nt_inv_matrix is None:
            nt_inv_matrix = self.get_amplitude_covariance(spec_params,
    ...
    def logprior(self, spec_params):
        ...
        #Top-hat priors
        if any(np.fabs(spec_params[self.id_tophat] - self.var_prior_meant) >
               self.var_prior_widtht):
            return None
        #Gausian priors
        return -0.5*np.sum(((spec_params[self.id_gauss]-self.var_prior_meang) *
                            self.var_prior_iwidthg) ** 2)

    def marginal_spectral_likelihood(self, spec_params, inst_params=None, add_prior=True):
        ...
        if add_prior:
            lprior = self.logprior(spec_params)
        else:
            lprior = 0
        if lprior is None:
            return -np.inf
        ...
        amp_covar_matrix = self.get_amplitude_covariance(spec_params,
                                                         inst_params, f_matrix)
        ...
        #   - This could be also written as (N^-1*d)^T*F^T*amp_mean, which
        # might save some time
        like = 0.5 * np.einsum("ij,ijk,ik->", amp_mean, amp_covar_matrix,
                               amp_mean) #TODO: check/optimize
        like += lprior
        if np.isnan(like):
            logging.warning("nan values in f_matrix.")
            return -np.inf
        return like

    def chi2(self, spec_params, inst_params=None, f_matrix=None):
        r""" Function to calculate the chi2 of a given set of spectral
        parameters.
    ...
    def chi2perdof(self, spec_params):
        r""" Function to calculate the reduced chi2 of a given set of spectral
        parameters.
        ...
        """
        chi2 = self.chi2(spec_params, inst_params=None, f_matrix=None)
        return chi2 / float(self.dof)

    def pval(self, spec_params, f_matrix=None):
        ...
        """
        chi2 = self.chi2(spec_params, f_matrix=f_matrix)
